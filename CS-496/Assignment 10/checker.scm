#lang eopl
  ...
  (define type-of
    (lambda (exp tenv)
      (cases expression exp
        ...
        (newref-exp (exp)
          (let ((t1 (type-of exp tenv)))
             (refto-type t1)))
        
        (deref-exp (exp)
          (let ((type1 (type-of exp tenv)))
            (cases type type1
              (refto-type (arg-type) arg-type)
              (else (report-deref-not-ref exp)))))
        
        (setref-exp (exp1 exp2)
          (let ((type1 (type-of exp1 tenv)))
            (cases type type1
              (refto-type (arg-type) (void-type))
              (else (report-deref-not-ref exp1)))))
  ...
  (define report-deref-not-ref
    (lambda (arg)
      (eopl:error 'type-of-expression "Argument not a valid reference: ~s" arg)))
