...
in let times4 = (fix t4m)
 ...
 (check-equal? (run "let x = newref(10) in let y = setref(x, 15) in deref(x)") 15 "deref test")
 (check-equal? (run "let x = newref(10) in x") 0 "newref test")
 (check-equal? (run "let x = newref(10) in let y = setref(x, 15) in begin -(1,1); deref(x) end") 15 "simple begin test")
 (check-equal? (run "let x = newref(10) in begin let y = setref(x,20) in 1; deref(x) end") 20 "begin test with let")
 (check-equal? (run "let x = newref(10) in 
                       let y = proc(x:refto int) 
                                deref(x) in 
                       begin let z = setref(x,20) in 1; (y x) end") 20 "procs with refs")
