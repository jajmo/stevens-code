...
public class RestMethod {
    ...
    public Response perform(PostMessageRequest request) {
        URL url = postMessageURL(request.senderId);
        if (url == null) {
    ...
    private URL registerURL(String chatname) {
        try {
            return new URL("http://10.0.2.2:8080/chat?chat-name=" + chatname);
        } catch (MalformedURLException e) {
            return null;
        }
    }

    private URL postMessageURL(long id) {
        try {
            return new URL("http://10.0.2.2:8080/chat/" + id + "/messages");
        } catch (MalformedURLException e) {
            return null;
        }
    }
