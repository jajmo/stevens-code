...
public class RestMethod {
    ...
    public StreamingResponse perform(SynchronizeRequest request, StreamingOutput out) throws IOException {
        URL url = syncMessagesURL(request.lastSequenceNumber);
        if (url == null) {
    ...
    private URL syncMessagesURL(long lastSeqNum) {
        if (syncMessagesUri == null) {
            long senderId = Settings.getSenderId(context);
            String uri = String.format("%s/%d/sync?last-seq-num=%d", baseUri, senderId, lastSeqNum);
