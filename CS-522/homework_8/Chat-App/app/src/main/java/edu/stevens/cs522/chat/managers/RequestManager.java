...
public class RequestManager extends Manager<ChatMessage> {
    ...
    public long getLastSequenceNumber() {
        String selection = MessageContract.SEQUENCE_NUM + "<>0";
        String[] selectionArgs = { };
        String[] columns = { String.format("MAX(%s) as %s", MessageContract.SEQUENCE_NUM, MAX_SEQNO_COLUMN) };
    ...
    public void updateSeqNum(long id, long seqNum) {
        ...
        ContentValues values = new ContentValues();
        values.put(MessageContract.SEQUENCE_NUM, seqNum);
    ...
    public long getMessageID(ChatMessage m, Context c) {
        long id = -1;

        if (m.senderId == Settings.getSenderId(c)) {
            String selection = MessageContract.CHAT_ROOM + "=? AND " + MessageContract.TIMESTAMP + "=?";
            String[] selectionArgs = { m.chatRoom, Long.toString(m.timestamp.getTime()) };
            Cursor c2 = getSyncResolver().query(MessageContract.CONTENT_URI, MessageContract.COLUMNS, selection, selectionArgs, null);
            if (c2.moveToFirst())
                id = MessageContract.getId(c2);
