...
int MyAmGraph<Elem>::firstNeighbor(unsigned int i)
{
    ...
    if (i < MyGraph<Elem>::v)
    {
        ...
        if (MyGraph<Elem>::directed)
        {
            ...
            {
                if (i != w && am[i][w] != MyGraph<Elem>::INF && am[i][w] != 0)
                    return w;
            }
        }
        else
        {
            for (w = 0; w < i; w++)
                if (am[w][i - w - 1] != MyGraph<Elem>::INF && am[w][i - w - 1] != 0)
                    return w;

            for (w = 0; w < (MyGraph<Elem>::v - i - 1); w++)
                if (am[i][w] != MyGraph<Elem>::INF && am[i][w] != 0)
                    return i + w + 1;
        }
    ...
    if ((i < MyGraph<Elem>::v) && (j < MyGraph<Elem>::v))
    {
        ...
        */
        ...
        else
        {
            ...
            {
                if (w != i && am[i][w - i - 1] != MyGraph<Elem>::INF && am[i][w - i - 1] != 0)
                    return w;
            }
        }
...
bool MyAmGraph<Elem>::add_edge(unsigned int i, unsigned int j, int weight)
{
    ...
    {
        if((this->get_edge(i, j) == this->INF) && (weight != this->INF))
            this->e++;

        if((weight == this->INF) && (get_edge(i, j) != this->INF))
            this->e--;
